...
void DirectionalLightCollectionD3D11::Initialize(ID3D11Device* device, const DirectionalLightData& lightInfo)
{
...
void DirectionalLightCollectionD3D11::UpdateLightBuffers(ID3D11DeviceContext* context, DirectX::XMFLOAT3 lightDir)
{

    DirectX::XMFLOAT3 centerPos = { 5, 0, -5 };
	
	for (int index = 0; index < shadowCameras.size(); index++)
	{
        bufferData[index].direction = lightDir;

		shadowCameras[index].UpdateInternalOrthographicConstantBuffer(context, centerPos, 100,
			bufferData[index].direction);

        bufferData[index].vpMatrix = shadowCameras[index].GetViewOrthographicProjectionMatrix(centerPos, 100,
            bufferData[index].direction);
        bufferData[index].position = centerPos;
	}
    lightBuffer.UpdateBuffer(context, bufferData.data());
}

UINT DirectionalLightCollectionD3D11::GetNrOfLights() const
{
	return bufferData.size();
}

ID3D11DepthStencilView* DirectionalLightCollectionD3D11::GetShadowMapDSV(UINT lightIndex) const
{
	return shadowMaps.GetDSV(lightIndex);
}

ID3D11ShaderResourceView* DirectionalLightCollectionD3D11::GetShadowMapsSRV() const
{
	return shadowMaps.GetSRV();
}

ID3D11ShaderResourceView* DirectionalLightCollectionD3D11::GetLightBufferSRV() const
{
	return lightBuffer.GetSRV();
}

ID3D11Buffer* DirectionalLightCollectionD3D11::GetLightCameraConstantBuffer(UINT lightIndex) const
{
	return shadowCameras[lightIndex].GetConstantBuffer();
}

D3D11_VIEWPORT DirectionalLightCollectionD3D11::GetViewport() const
{
	return viewport;
}

void DirectionalLightCollectionD3D11::BindDirectionallights(ID3D11DeviceContext* context)
{
	ID3D11ShaderResourceView* srv = lightBuffer.GetSRV();  // Get the SRV from your structured buffer
	if (srv)
	{
		context->CSSetShaderResources(6, 1, &srv); // Bind SRV to slot 3
	}
	ID3D11ShaderResourceView* srvShadowMap = shadowMaps.GetSRV();
	if (srvShadowMap)
	{
		context->CSSetShaderResources(7, 1, &srvShadowMap); // Bind SRV to slot 4
	}
...
void DirectionalLightCollectionD3D11::UnBindDirectionalLights(ID3D11DeviceContext* context)
{
